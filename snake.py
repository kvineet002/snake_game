...
class Snake:
    def __init__(self):
        ...
        self.direction = pygame.Vector2(1, 0)  # Start moving right
        self.has_eaten = False
        self.move_delay = 20  # Movement delay in milliseconds
    ...
    def import_surfs(self):
        surf_dict = {}
        for folder_path, _, image_names in walk(path.join('.', 'graphics', 'snake')):
            for image_name in image_names:
                full_path = path.join(folder_path, image_name)
    ...
    def reset(self):
        self.body = [pygame.Vector2(START_COL - col, START_ROW) for col in range(START_LENGTH)]
        self.direction = pygame.Vector2(1, 0)  # Reset to start moving right
        self.has_eaten = False
        self.last_move_time = pygame.time.get_ticks()
